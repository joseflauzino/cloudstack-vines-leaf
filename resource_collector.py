...
class ResourceCollector(object):
    ...
    def _get_cpu_time(self):
        ...
        Algorithm used is based on https://stackoverflow.com/questions/23367857/accurate-calculation-of-cpu-usage-given-in-percentage-in-linux/
        """
    ...
    def get_cpu_usage(self):
        ...
        for _ in range(self.test_duration):
            start = self._get_cpu_time()
            time.sleep(self.sleeptime)
            stop = self._get_cpu_time()
        ...
        cpu_usage = total/self.test_duration
        return cpu_usage
    ...
    def get_bandwidth_usage(self):
        ...
        # parse dstat output
        for _ in bwidth:
            rx, tx = [self._parse_units(v) for v in ' '.join(re.sub('\x1b.*?m', '', _).split()).split(' ')]
